...
(defun zb:zebu-compiler (&key (compiled t) (verbose t))
  ...
  (let ((defaults 
    ...
    (dolist (name '("kb-dom" "regex"
                    "oset" "g-symbol" "loadgram"
                    "gnrator" "lr0-sets"
                    "empty-st" "first"
                    "follow" "tables"
                    "slr" "closure"
    ...
    (ZB:zebu-compile-file
     (merge-pathnames (make-pathname :name "mg" :type "zb")
                      *ZEBU-directory*)
     :output-file (merge-pathnames (make-pathname :name "mg")
                                   *ZEBU-binary-directory*)
     :verbose verbose
     :compile-domain compiled)
    (zb::zebu-load-file (merge-pathnames
                         (make-pathname :name "mg" :type "tab")
                         *ZEBU-binary-directory*))
