...
(defmethod word-equal ((token tok-eol) (other (eql 'eol))) t)
...
;;       (word-equal (make-instance 'tok-eof) 'eof))
;; (t nil t t)
  ...
  :rules (
          ...
          (--> simple
               (alt
                ...
                (seq procident
                     (opt tok-pargauche (opt liste-argument :action $1) tok-pardroite
                          :action (list $2))
                     :action (if $2
          ...
          (--> reference
               (seq identificateur
                    (opt (alt (seq tok-crogauche expression
          ...
          (--> liste-reference
               (seq reference (rep (seq tok-virgule reference :action $2) :action $1)
                    :action (cons $1 $2))
               :action $1)
